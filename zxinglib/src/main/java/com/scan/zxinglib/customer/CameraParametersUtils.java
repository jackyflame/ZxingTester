...
public class CameraParametersUtils {
    ...
    public void getCameraPreParameters(Camera camera){
        ...
        for (int i = 0; i < supportSizeList.size(); i++) {
            ...
            // 判断屏幕宽高比是否与预览宽高比一样，如果一样执行如下代码
            if (ratioScreen == ratioPreview) {
                // 默认预览以1280*720为标准
                if (supportSizeList.get(i).width >= 1280 || supportSizeList.get(i).height >= 720) {
                    // 初始值
                    ...
                    if (supportSizeList.get(0).width > supportSizeList.get(supportSizeList.size() - 1).width) {
                        // 如果第一个值大于最后一个值
                        if (preWidth > supportSizeList.get(i).width || preHeight > supportSizeList.get(i).height) {
